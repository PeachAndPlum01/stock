# 股票投资系统 - 微服务快速启动指南

## 🎉 项目已完成微服务改造！

本项目已成功改造为标准的微服务架构，包含5个独立的微服务模块。

## 📦 服务列表

| 服务名称 | 端口 | 状态 | 说明 |
|---------|------|------|------|
| stock-gateway | 8080 | ✅ 完成 | API网关，统一入口 |
| stock-auth-service | 8081 | ✅ 完成 | 认证服务，用户登录注册 |
| stock-data-service | 8082 | ✅ 完成 | 股票数据服务，Tushare集成 |
| stock-investment-service | 8083 | ✅ 完成 | 投资信息服务，股票查询 |
| stock-correlation-service | 8084 | ✅ 完成 | 省份关联服务，相关度分析 |

## 🚀 快速启动（3种方式）

### 方式1：Docker Compose 一键启动（推荐）

```bash
# 1. 配置环境变量
cp .env.template .env
# 编辑.env文件，设置TUSHARE_TOKEN

# 2. 启动所有服务
docker-compose up -d

# 3. 查看服务状态
docker-compose ps

# 4. 查看日志
docker-compose logs -f stock-gateway

# 5. 停止服务
docker-compose down
```

### 方式2：Maven 本地启动

```bash
# 1. 启动基础设施（MySQL、Redis、RabbitMQ）
docker-compose up -d mysql redis rabbitmq

# 2. 等待数据库初始化完成（约30秒）
sleep 30

# 3. 编译项目
mvn clean package -DskipTests

# 4. 启动各个服务（按顺序）
# 终端1 - 启动网关
cd stock-gateway
mvn spring-boot:run

# 终端2 - 启动认证服务
cd stock-auth-service
mvn spring-boot:run

# 终端3 - 启动股票数据服务
cd stock-data-service
mvn spring-boot:run

# 终端4 - 启动投资信息服务
cd stock-investment-service
mvn spring-boot:run

# 终端5 - 启动省份关联服务
cd stock-correlation-service
mvn spring-boot:run
```

### 方式3：使用启动脚本

```bash
# 启动所有服务
./start-all.sh

# 停止所有服务
./stop-all.sh
```

## 🔍 验证服务是否启动成功

### 1. 检查服务健康状态

```bash
# 网关
curl http://localhost:8080/actuator/health

# 认证服务
curl http://localhost:8081/actuator/health

# 股票数据服务
curl http://localhost:8082/actuator/health

# 投资信息服务
curl http://localhost:8083/actuator/health

# 省份关联服务
curl http://localhost:8084/actuator/health
```

### 2. 测试登录接口

```bash
curl -X POST http://localhost:8080/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "username": "admin",
    "password": "123456"
  }'
```

成功响应示例：
```json
{
  "code": 200,
  "message": "登录成功",
  "data": {
    "token": "eyJhbGciOiJIUzI1NiJ9...",
    "userId": 1,
    "username": "admin",
    "nickname": "管理员"
  }
}
```

### 3. 测试股票查询接口（需要Token）

```bash
# 先登录获取token
TOKEN=$(curl -s -X POST http://localhost:8080/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"123456"}' \
  | jq -r '.data.token')

# 查询股票列表
curl -X GET "http://localhost:8080/api/investment/list?page=1&size=10" \
  -H "Authorization: Bearer $TOKEN"

# 查询涨幅榜
curl -X GET "http://localhost:8080/api/investment/top/gainers?limit=5" \
  -H "Authorization: Bearer $TOKEN"

# 查询省份统计
curl -X GET "http://localhost:8080/api/investment/stats/province" \
  -H "Authorization: Bearer $TOKEN"
```

## 📊 API接口文档

### 认证服务 API

#### 1. 用户登录
```http
POST /api/auth/login
Content-Type: application/json

{
  "username": "admin",
  "password": "123456"
}
```

#### 2. 用户注册
```http
POST /api/auth/register
Content-Type: application/json

{
  "username": "newuser",
  "password": "123456",
  "nickname": "新用户",
  "email": "user@example.com"
}
```

#### 3. 获取用户信息
```http
GET /api/auth/userinfo
Authorization: Bearer {token}
```

#### 4. 用户登出
```http
POST /api/auth/logout
Authorization: Bearer {token}
```

### 投资信息服务 API

#### 1. 分页查询股票列表
```http
GET /api/investment/list?page=1&size=20&province=京&keyword=人工智能&sortBy=tenDayChange&sortOrder=desc
Authorization: Bearer {token}
```

#### 2. 获取股票详情
```http
GET /api/investment/{id}
Authorization: Bearer {token}
```

#### 3. 获取涨幅榜
```http
GET /api/investment/top/gainers?limit=10&province=京
Authorization: Bearer {token}
```

#### 4. 按省份统计
```http
GET /api/investment/stats/province
Authorization: Bearer {token}
```

#### 5. 按涨幅范围查询
```http
GET /api/investment/range?minChange=10&maxChange=20
Authorization: Bearer {token}
```

#### 6. 按题材查询
```http
GET /api/investment/theme/人工智能
Authorization: Bearer {token}
```

#### 7. 获取所有省份
```http
GET /api/investment/provinces
Authorization: Bearer {token}
```

### 省份关联服务 API

#### 1. 获取指定省份的相关省份
```http
GET /api/correlation/province/京
Authorization: Bearer {token}
```

#### 2. 获取两个省份之间的相关度
```http
GET /api/correlation/between?source=京&target=沪
Authorization: Bearer {token}
```

#### 3. 获取相关度最高的省份对
```http
GET /api/correlation/top?limit=10
Authorization: Bearer {token}
```

#### 4. 按相关度范围查询
```http
GET /api/correlation/range?minScore=0.5&maxScore=1.0
Authorization: Bearer {token}
```

#### 5. 按共同概念查询
```http
GET /api/correlation/concept/人工智能
Authorization: Bearer {token}
```

### 股票数据服务 API

#### 1. 获取实时行情
```http
GET /api/stock/realtime/quote?symbol=688981.SH
Authorization: Bearer {token}
```

#### 2. 启动实时推送
```http
POST /api/stock/realtime/start
Authorization: Bearer {token}
```

#### 3. 停止实时推送
```http
POST /api/stock/realtime/stop
Authorization: Bearer {token}
```

## 🔧 配置说明

### 环境变量配置（.env文件）

```bash
# Tushare API Token（必须配置）
TUSHARE_TOKEN=your_tushare_token_here

# MySQL配置
MYSQL_ROOT_PASSWORD=root
MYSQL_DATABASE=stock_investment

# Redis配置
REDIS_PASSWORD=

# RabbitMQ配置
RABBITMQ_DEFAULT_USER=guest
RABBITMQ_DEFAULT_PASS=guest

# JWT配置
JWT_SECRET=stock-investment-system-jwt-secret-key-2024
JWT_EXPIRATION=86400000
```

### 端口配置

| 服务 | 端口 | 说明 |
|------|------|------|
| API网关 | 8080 | 统一入口 |
| 认证服务 | 8081 | 用户认证 |
| 股票数据服务 | 8082 | Tushare数据 |
| 投资信息服务 | 8083 | 股票查询 |
| 省份关联服务 | 8084 | 相关度分析 |
| MySQL | 3306 | 数据库 |
| Redis | 6379 | 缓存 |
| RabbitMQ | 5672 | 消息队列 |
| RabbitMQ管理界面 | 15672 | Web管理 |

## 📁 项目结构

```
stock/
├── pom.xml                          # 父POM
├── docker-compose.yml               # Docker编排
├── .env.template                    # 环境变量模板
├── start-all.sh                     # 启动脚本
├── stop-all.sh                      # 停止脚本
├── sql/
│   └── init.sql                     # 数据库初始化
├── frontend/                        # 前端项目
├── stock-gateway/                   # ✅ API网关
├── stock-auth-service/              # ✅ 认证服务
├── stock-data-service/              # ✅ 股票数据服务
├── stock-investment-service/        # ✅ 投资信息服务
└── stock-correlation-service/       # ✅ 省份关联服务
```

## 🐛 常见问题

### 1. 服务启动失败

**问题**：服务启动时报错连接数据库失败

**解决**：
```bash
# 检查MySQL是否启动
docker-compose ps mysql

# 查看MySQL日志
docker-compose logs mysql

# 等待MySQL完全启动（约30秒）
sleep 30
```

### 2. Token验证失败

**问题**：请求API时返回401 Unauthorized

**解决**：
- 检查Token是否正确
- 检查Token是否过期（默认24小时）
- 确保请求头格式正确：`Authorization: Bearer {token}`

### 3. 端口冲突

**问题**：服务启动时报错端口已被占用

**解决**：
```bash
# 查看端口占用
lsof -i :8080

# 修改配置文件中的端口
vim stock-gateway/src/main/resources/application.yml
```

### 4. Tushare API调用失败

**问题**：股票数据服务无法获取数据

**解决**：
- 检查TUSHARE_TOKEN是否配置正确
- 确认Tushare账号是否有权限
- 检查网络连接

## 📚 相关文档

- [微服务架构说明](./ 微服务架构说明.md) - 微服务架构详细说明
- [微服务改造完成报告](./ 微服务改造完成报告.md) - 改造完成报告
- [API接口文档](./API接口文档.md) - 完整API文档
- [部署文档](./ 部署文档.md) - 部署文档

## 🎯 下一步

1. **前端集成**：修改前端API基础地址为网关地址（http://localhost:8080）
2. **服务发现**：集成Nacos实现服务自动注册与发现
3. **配置中心**：使用Nacos Config统一管理配置
4. **监控告警**：集成Prometheus + Grafana
5. **链路追踪**：集成Sleuth + Zipkin

## 💡 技术栈

- **框架**：Spring Boot 2.7.18、Spring Cloud 2021.0.8
- **网关**：Spring Cloud Gateway
- **数据库**：MySQL 8.0、MyBatis Plus 3.5.4
- **缓存**：Redis 7.0
- **消息队列**：RabbitMQ 3.12
- **认证**：JWT (jjwt 0.11.5)
- **容器化**：Docker、Docker Compose

## 📞 支持

如有问题，请查看：
1. 服务日志：`docker-compose logs -f {service-name}`
2. 健康检查：`curl http://localhost:{port}/actuator/health`
3. 项目文档：查看docs目录

---

**项目版本**：1.0.0  
**最后更新**：2026-02-01  
**微服务改造**：✅ 已完成
