# 天问 - 微服务版

## 项目简介
这是一个基于微服务架构的天问系统，支持基于中国地图的可视化投资信息展示。采用Spring Cloud微服务技术栈，实现服务解耦和独立部署。

## 🎯 项目状态

**当前版本：微服务架构 v1.0.0（✅ 已完成）**

### ✅ 已完成（2026-02-01）
- ✅ 微服务目录结构
- ✅ 父POM配置
- ✅ API网关服务（stock-gateway）
- ✅ 认证服务（stock-auth-service）
- ✅ 股票数据服务（stock-data-service）
- ✅ 投资信息服务（stock-investment-service）
- ✅ 省份关联服务（stock-correlation-service）
- ✅ Docker容器化支持
- ✅ 完整的API接口
- ✅ 详细的项目文档

**代码统计**：30个Java文件，5个配置文件，5个Dockerfile，8个文档文件

## 🏗️ 微服务架构

```
前端 (5173)
    ↓
API网关 (8080)
    ↓
    ├─→ 认证服务 (8081)
    ├─→ 股票数据服务 (8082)
    ├─→ 投资信息服务 (8083)
    └─→ 省份关联服务 (8084)
```

## 📦 服务列表

| 服务名称 | 端口 | 状态 | 说明 |
|---------|------|------|------|
| stock-gateway | 8080 | ✅ | API网关、路由转发、JWT验证 |
| stock-auth-service | 8081 | ✅ | 用户认证、登录注册、Token管理 |
| stock-data-service | 8082 | ✅ | 股票数据获取、Tushare API、实时推送 |
| stock-investment-service | 8083 | ✅ | 投资信息查询、统计分析 |
| stock-correlation-service | 8084 | ✅ | 省份关联度计算、相关度分析 |

## 技术栈

### 后端（微服务）
- **微服务框架**: Spring Boot 2.7.18 + Spring Cloud 2021.0.8
- **API网关**: Spring Cloud Gateway
- **数据库**: MySQL 8.0 + MyBatis Plus
- **缓存**: Redis
- **消息队列**: RabbitMQ
- **认证**: JWT
- **股票数据**: Tushare API

### 前端
- Vue 3
- Element Plus
- ECharts 5（中国地图）
- Axios
- Vue Router
- Pinia

## 项目结构
```
stock-microservices/
├── pom.xml                          # 父POM，统一管理依赖版本
├── docker-compose.yml               # Docker Compose编排文件
├── .env.template                    # 环境变量配置模板
├── start-all.sh                     # 启动所有服务脚本
├── stop-all.sh                      # 停止所有服务脚本
├── stock-gateway/                   # API网关 (8080) ✅
│   ├── src/main/java/com/stock/gateway/
│   │   ├── GatewayApplication.java
│   │   ├── config/CorsConfig.java
│   │   └── filter/AuthFilter.java
│   ├── src/main/resources/application.yml
│   ├── Dockerfile
│   └── pom.xml
├── stock-auth-service/              # 认证服务 (8081) ✅
│   ├── src/main/java/com/stock/auth/
│   │   ├── AuthServiceApplication.java
│   │   ├── entity/User.java
│   │   ├── mapper/UserMapper.java
│   │   ├── service/AuthService.java
│   │   ├── controller/AuthController.java
│   │   ├── util/JwtUtil.java
│   │   └── config/RedisConfig.java
│   ├── src/main/resources/application.yml
│   ├── Dockerfile
│   └── pom.xml
├── stock-data-service/              # 股票数据服务 (8082) ✅
│   ├── src/main/java/com/stock/data/
│   │   ├── StockDataServiceApplication.java
│   │   ├── client/TushareApiClient.java
│   │   ├── service/StockRealtimeService.java
│   │   ├── controller/StockDataController.java
│   │   ├── model/StockQuote.java
│   │   ├── rabbitmq/
│   │   ├── config/
│   │   └── common/Result.java
│   ├── src/main/resources/application.yml
│   ├── Dockerfile
│   └── pom.xml
├── stock-investment-service/        # 投资信息服务 (8083) ✅
│   ├── src/main/java/com/stock/investment/
│   │   ├── InvestmentServiceApplication.java
│   │   ├── entity/InvestmentInfo.java
│   │   ├── mapper/InvestmentInfoMapper.java
│   │   ├── service/InvestmentInfoService.java
│   │   └── controller/InvestmentInfoController.java
│   ├── src/main/resources/application.yml
│   ├── Dockerfile
│   └── pom.xml
├── stock-correlation-service/       # 省份关联服务 (8084) ✅
│   ├── src/main/java/com/stock/correlation/
│   │   ├── CorrelationServiceApplication.java
│   │   ├── entity/RelatedProvince.java
│   │   ├── mapper/RelatedProvinceMapper.java
│   │   ├── service/CorrelationService.java
│   │   └── controller/CorrelationController.java
│   ├── src/main/resources/application.yml
│   ├── Dockerfile
│   └── pom.xml
├── frontend/                        # 前端项目 (5173)
│   ├── src/
│   │   ├── components/
│   │   ├── views/
│   │   ├── router/
│   │   ├── store/
│   │   └── api/
│   └── package.json
├── sql/                            # 数据库脚本
│   └── init.sql
├── 项目说明.md                       # 项目说明
├── 快速启动指南.md                   # 快速启动指南 ⭐
├── 微服务架构说明.md                # 微服务架构说明
├── 微服务改造总结.md        # 微服务改造总结 ⭐
├── 微服务改造完成报告.md # 改造完成报告
├── API接口文档.md                          # API文档
└── 部署文档.md                       # 部署文档
```

## 功能特性
1. 用户登录认证
2. 基于中国地图的投资信息可视化
3. 省份点击交互，显示该省份近期10条投资信息
4. 关联省份高亮提示
5. 实时数据更新（通过消息队列）

## 快速开始

### 环境要求
- JDK 1.8+ (推荐使用JDK 8/11/17/21/25)
  - 当前系统JDK: OpenJDK 25.0.1
  - 项目编译目标: Java 1.8
  - Maven已配置向下兼容，可使用高版本JDK编译Java 8代码
- Node.js 16+
- MySQL 8.0+
  - 默认用户: root
  - 默认密码: root
- Redis 5.0+
- RabbitMQ 3.8+
- Docker & Docker Compose (可选，用于容器化部署)

### 方式一：Docker Compose部署（推荐）

#### 1. 配置环境变量
```bash
# 复制环境配置模板
cp .env.template .env

# 编辑.env文件，填写实际配置（特别是TUSHARE_TOKEN）
vim .env
```

#### 2. 启动所有服务
```bash
# 构建并启动所有服务（包括MySQL、Redis、RabbitMQ和所有微服务）
docker-compose up -d

# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f
```

#### 3. 停止服务
```bash
docker-compose down
```

### 方式二：本地部署

#### 1. 数据库初始化
```bash
mysql -u root -p < sql/init.sql
```

#### 2. 配置Tushare Token（可选）
编辑 `stock-data-service/src/main/resources/application.yml`：
```yaml
tushare:
  token: your_tushare_token_here
```

#### 3. 启动所有微服务
```bash
# 使用启动脚本（推荐）
chmod +x start-all.sh
./start-all.sh

# 或手动启动各服务
mvn clean package -DskipTests
cd stock-gateway && mvn spring-boot:run &
cd stock-auth-service && mvn spring-boot:run &
cd stock-data-service && mvn spring-boot:run &
cd stock-investment-service && mvn spring-boot:run &
cd stock-correlation-service && mvn spring-boot:run &
```

#### 4. 启动前端
```bash
cd frontend
npm install
npm run dev
```

#### 5. 访问系统
- 前端: http://localhost:5173
- API网关: http://localhost:8080
- 默认账号: admin / 123456

## 📚 项目文档

- **[快速启动指南](快速启动指南.md)** ⭐ - 3分钟快速上手
- **[微服务改造总结](微服务改造总结.md)** ⭐ - 改造成果和技术细节
- **[微服务架构说明](微服务架构说明.md)** - 架构设计和服务说明
- **[改造完成报告](微服务改造完成报告.md)** - 详细的改造过程
- **[微服务架构检查报告](微服务架构检查清单.md)** - 架构标准检查和评分
- **[API文档](API接口文档.md)** - API接口文档
- **[部署文档](部署文档.md)** - 部署指南

## 🔧 配置说明

### JWT配置
网关和认证服务需要使用相同的JWT密钥：
```yaml
jwt:
  secret: stock-investment-system-jwt-secret-key-2024
  expiration: 86400000
```

### 数据库配置
各服务独立配置数据库连接：
```yaml
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/stock_db
    username: root
    password: root
```

### Redis配置
```yaml
spring:
  redis:
    host: localhost
    port: 6379
```

### RabbitMQ配置
```yaml
spring:
  rabbitmq:
    host: localhost
    port: 5672
    username: guest
    password: guest
```

## 访问地址
- 前端: http://localhost:5173
- API网关: http://localhost:8080
- 认证服务: http://localhost:8081
- 股票数据服务: http://localhost:8082
- 投资信息服务: http://localhost:8083
- 省份关联服务: http://localhost:8084
- 默认账号: admin / 123456

## 主要功能说明

### 地图交互
- 点击省份：该省份视觉上浮起，显示投资信息
- 关联提示：高亮显示有相关投资信息的其他省份
- 数据展示：显示该省份近期10条投资信息

### 微服务功能
- ✅ 用户登录认证（JWT）
- ✅ API网关统一入口
- ✅ 服务独立部署
- ✅ 实时股票数据获取
- ✅ Redis缓存优化
- ✅ RabbitMQ消息队列
- ✅ 服务健康检查

## 🐛 故障排查

### 服务启动失败
```bash
# 查看服务日志
tail -f stock-*/logs/*.log

# 检查端口占用
lsof -i :8080
```

### 网关路由不通
```bash
# 检查目标服务是否启动
curl http://localhost:8081/actuator/health
curl http://localhost:8082/actuator/health
```

### 数据库连接失败
```bash
# 检查MySQL是否启动
mysql -u root -p -e "SELECT 1"
```

## 日志系统

每个微服务都有独立的日志文件，存储在各自的 `logs/` 目录下：

```bash
# 查看网关日志
tail -f stock-gateway/logs/gateway.log

# 查看认证服务日志
tail -f stock-auth-service/logs/auth.log

# 查看所有服务日志
tail -f stock-*/logs/*.log
```

## 📝 开发指南

### 添加新的微服务

1. 在父POM中添加模块
2. 创建服务目录和pom.xml
3. 实现服务代码
4. 在网关中配置路由
5. 更新启动脚本

### 服务间通信

- **同步通信**: HTTP/REST（通过网关）
- **异步通信**: RabbitMQ消息队列
- **数据共享**: Redis缓存

## 📈 性能优化

- Redis缓存热点数据
- 数据库索引优化
- 服务独立扩展
- 消息队列异步处理

## 🔐 安全特性

- JWT Token认证
- 密码加密存储
- API网关统一鉴权
- CORS跨域配置

## 🤝 贡献指南

欢迎提交Issue和Pull Request！

## 📄 许可证

MIT License
